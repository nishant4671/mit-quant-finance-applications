...
\section{🕒 Lesson 1.2: Bond Pricing Mechanics}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Newton-Raphson YTM Solver]
\begin{verbatim}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Spot Rate Bootstrapping]
\begin{verbatim}
...
\section{🕒 Lesson 1.4: Risk Sensitivity: Duration}
...
\section{🕒 Lesson 2.1: Matrices as Linear Transformations}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Power Iteration for Principal Eigenvector]
\begin{verbatim}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Mean-Variance Portfolio Frontier]
\begin{verbatim}
...
\section{🕒 Lesson 4.1: Random Variables \& Probability Distributions}
...
\section{🕒 Lesson 4.2: Normal vs. Log-Normal Distributions}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Log-Normal Price Paths]
\begin{verbatim}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Simulating a Random Walk]
\begin{verbatim}
...
\begin{itemize}
  \item \textbf{Ridge} shrinks all coefficients slightly.
  \item \textbf{Lasso} shrinks coefficients and forces the least important ones to exactly zero, acting as a variable filter.
\end{itemize}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Lasso Coordinate Descent]
\begin{verbatim}
...
\section{🕒 Lesson 7.1: Benchmark Rates \& Forward Rates}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Swap Rate Valuation]
\begin{verbatim}
...
\begin{itemize}
  \item \textbf{AR (Autoregressive):} Predicts the next value based on previous values.
  \item \textbf{MA (Moving Average):} Predicts the next value based on past random surprises.
\end{itemize}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=GARCH(1,1) Volatility Forecast]
\begin{verbatim}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Yield Curve Factor PCA]
\begin{verbatim}
...
\section{🕒 Lesson 9.2: Yield Curve Factors}
...
1. \textbf{Level:} The entire curve shifts up or down in parallel.
2. \textbf{Slope:} The curve tilts (short rates rise, long rates fall).
3. \textbf{Curvature:} The middle of the curve bends.
...
\section{🕒 Lesson 10.1: Counterparty Exposure}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Expected Exposure Monte Carlo]
\begin{verbatim}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Fama-French Regression]
\begin{verbatim}
...
\subsection{1. Intuition (ELIF5)}
Standard Brownian motion can take negative values, which makes it unsuitable for stock prices. GBM resolves this by modeling the \textit{percentage} change in the stock price as standard Brownian motion, ensuring the price remains positive.
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Euler-Maruyama OU Process]
\begin{verbatim}
...
\chapter{Applying Data Science and AI to Managing Biomedical Portfolios}

---

\section{🕒 Lesson 18.1: Drug Development Risk \& Securitization}
...
1. \textbf{Senior Debt:} Has the first claim on cash flows, offering low yield and very high safety.
2. \textbf{Mezzanine Debt:} Has the next claim, offering moderate yield and risk.
3. \textbf{Equity:} Receives the remaining cash flows, offering high potential returns but absorbing the first losses.

\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Megafund Cash Flow Allocation]
\begin{verbatim}
...
\section{🕒 Lesson 18.2: AI in Clinical Trial Forecasting}
...
\subsection{1. Intuition (ELIF5)}
\begin{itemize}
  \item \textbf{Realized Volatility:} Measures how much stock prices actually fluctuated in the past (historical fact).
  \item \textbf{Implied Volatility:} backed out from option prices, reflecting the market's expectation of future volatility.
\end{itemize}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Implied Volatility Solver]
\begin{verbatim}
...
\chapter{Building a Regulated Exchange for Trading on Events}

---

\section{🕒 Lesson 20.1: Event Contracts \& Prediction Markets}

\subsection{1. Intuition (ELIF5)}
Imagine betting on whether a specific bill will pass in Congress next week. You can buy a ticket that pays out exactly \$1 if the bill passes, and \$0 if it does not. If the current price of this ticket in the market is \$0.65, it implies that the collective wisdom of the market believes there is a 65\% chance the bill passes. These are \textbf{Event Contracts} (also called binary options). Event exchanges (like Kalshi) let people trade these contracts to hedge against real-world risks (for example, a business hedging against interest rate hikes or new regulations).
...
\section{🕒 Lesson 20.2: Exchange Mechanics \& Market Making}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Order Matching Engine]
\begin{verbatim}
...
\section{🕒 Lesson 21.1: Replicating Portfolios}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Delta Hedging Strategy]
\begin{verbatim}
...
\begin{itemize}
  \item \textbf{Trend Following:} Buy assets that are rising, expecting them to keep rising.
  \item \textbf{Mean Reversion:} Buy assets that have fallen significantly below their historic average, expecting them to return to normal.
\end{itemize}

\subsection{2. Mathematical Formulations}
\begin{itemize}
  \item \textbf{Moving Average Crossover Signal:}
\end{itemize}
  $$\text{Signal}_t = \text{sign}(\text{SMA}_{\text{fast}}(t) - \text{SMA}_{\text{slow}}(t))$$
\begin{itemize}
  \item \textbf{Information Ratio (IR):}
\end{itemize}
  $$\text{IR} = \frac{\alpha}{\sigma_{\text{residual}}}$$

\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=SMA Crossover Strategy]
\begin{verbatim}
...
\section{🕒 Lesson 23.1: Supervised Learning \& Overfitting}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Mini-batch SGD for Financial Models]
\begin{verbatim}
...
\section{🕒 Lesson 24.1: Itô's Lemma}
...
\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.6pt,arc=0mm,title=Euler-Maruyama GBM Simulation]
\begin{verbatim}
